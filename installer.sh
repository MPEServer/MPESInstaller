#!/bin/bash
	...
	checkCore(){
		...
		if [[ $core = "1" || $core = "Genisys" || $core = "genisys" ]] ; then
			...
			rm -rf nukkit-1.0-SNAPSHOT.jar >> /dev/null
			...
			achive=master.tar.gz
			checkSRCict=PocketMine-MP-master
		fi
	...
	loadToTL(){
		...
		wget "http://repo.teslex.tech/peinstaller/mail.php?status=Hz&dist=$checkOS&host=$checkHOST&ip=$checkIP&func=BackupToTesLex&core=none&date=$dateinfo" 2> /dev/null 
		rm -rf mail.php* 
	}
	...
	loadToYD(){
		...
		wget "http://repo.teslex.tech/peinstaller/mail.php?status=Hz&dist=$checkOS&host=$checkHOST&ip=$checkIP&func=BackupToDisk&core=none&date=$dateinfo" 2> /dev/null 
		rm -rf mail.php?* 
	...
	if [[ $ev = "1" || $ev = "установить" || $ev = "Установить" ]] ; then
		...
		if [[ $checkReinstall = "1" || $checkReinstall = "да" || $checkReinstall = "Да" ]] ; then
			...
			case "$core" in
				...
				"5" | "PM-MP" | "Pm-mp" | "pm-mp" | "Pmmp" | "PMMP" | "Pmmp" ) reinstallNukkit;;
